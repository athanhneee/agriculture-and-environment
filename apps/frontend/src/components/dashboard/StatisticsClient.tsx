...
export function StatisticsClient({ initialOverview }: StatisticsClientProps) {
  ...
  const [alertsStats, setAlertsStats] = useState<any[]>([]);
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      setAlertsStats(alertsData.success && alertsData.data ? alertsData.data : []);
      setReadingsStats(readingsData || []);
    } catch (err: any) {
  ...
  const getSeverityData = () => {
    ...
    alertsStats.forEach((alert) => {
      const sev = alert.severity || "INFO";
      if (summary[sev] !== undefined) {
        summary[sev] += alert.count || 1;
      }
    });

    return Object.keys(summary)
      .map((key) => ({
        name: key === "CRITICAL" ? "Nguy cấp" : key === "WARNING" ? "Cảnh báo" : "Thông tin",
        value: summary[key],
        severity: key,
      }))
