...
export default function DashboardError({ error, reset }: DashboardErrorProps) {
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center text-center p-6">
      ...
      {error.message && (
        <pre className="mt-4 p-3 bg-muted text-xs text-destructive rounded-xl max-w-lg overflow-x-auto text-left font-mono">
          {error.message}
        </pre>
      )}
      ...
      <button
        onClick={reset}
        className="mt-6 inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 px-5 text-sm font-semibold text-white transition shadow-sm shadow-emerald-600/10"
      >
