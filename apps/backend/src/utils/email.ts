...
// Tạo transporter dùng cấu hình SMTP Gmail hoặc Ethereal Email (nếu không có cấu hình)
export const createTransporter = async () => {
  if (env.smtpUser && env.smtpPass) {
    return nodemailer.createTransport({
      service: 'gmail', // Sử dụng service gmail chuẩn
      auth: {
        user: env.smtpUser,
        pass: env.smtpPass, // Mật khẩu ứng dụng (App Password)
      },
    });
  }

  // Nếu không có cấu hình SMTP, sử dụng Ethereal để test (in ra console)
  console.warn('⚠️ SMTP (Gmail) chưa được cấu hình. Sử dụng Ethereal Email để test...');
...
export const sendOtpEmail = async (to: string, otp: string) => {
  try {
    ...
    console.error('❌ Lỗi khi gửi email qua Nodemailer/Gmail:', error);
