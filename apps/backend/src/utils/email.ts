...
// Tạo transporter dùng Ethereal Email (khi không có Resend API Key)
export const createTransporter = async () => {
  console.warn('⚠️ Resend API Key chưa được cấu hình. Sử dụng Ethereal Email để test...');
  ...
  try {
    if (resend) {
      ...
      if (error) {
        ...
        throw new Error('Không thể gửi email OTP qua Resend');
      }

      console.log('✅ Đã gửi email OTP thành công qua Resend!', data);
    } else {
      // Sử dụng Ethereal Email làm fallback
