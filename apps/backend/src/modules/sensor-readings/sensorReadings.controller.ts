...
export class SensorReadingController {
  static async getReadings(req: Request, res: Response) {
    const { farmZoneId, sensorId, from, to, page = "1", limit = "10" } =
      req.query as any;
    ...
    res
      .status(200)
      .json(ApiResponse.success("Sensor readings retrieved successfully", result));
  }

  static async getLatestReadings(req: Request, res: Response) {
    const latest = await SensorReadingService.getLatestReadings(req.user!);

    res
      .status(200)
      .json(ApiResponse.success("Latest sensor readings retrieved successfully", latest));
  }
  ...
  static async createReading(req: Request, res: Response) {
    ...
    res
      .status(201)
      .json(ApiResponse.success("Sensor reading created successfully", reading));
  }
  ...
  static async deleteReading(req: Request, res: Response) {
    ...
    res
      .status(200)
      .json(ApiResponse.success("Sensor reading deleted successfully", null));
  }
}
