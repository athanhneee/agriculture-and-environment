...
#### 📌 Bước 5: Test API Lấy Danh Sách (GET List)
*   **Method:** `GET`
*   **URL:** `http://localhost:5000/api/farm-zones?page=1&limit=5&search=Cà Chua`
*   **Kết quả:** Mã 200 OK. Trả về cấu trúc phân trang:
```json
{
  "success": true,
...
#### 📌 Bước 6: Test API Xem Chi Tiết Kèm Relations (GET Detail)
*   **Method:** `GET`
*   **URL:** `http://localhost:5000/api/farm-zones/<dán_id_farm_zone_từ_bước_2>`
*   **Kết quả:** Mã 200 OK. Bên trong `data` trả về sẽ chứa thêm mảng `crops`, `sensors`, và `alerts`.

#### 📌 Bước 7: Test API Xóa (DELETE)
*   **Method:** `DELETE`
*   **URL:** `http://localhost:5000/api/farm-zones/<dán_id_farm_zone_từ_bước_2>`
*   **Kết quả:** Mã 200 OK. Toàn bộ `crops` và `sensors` thuộc về Zone này cũng sẽ bị tự động xóa sạch (Cascade Delete).