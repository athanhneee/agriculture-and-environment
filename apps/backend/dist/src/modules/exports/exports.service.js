...
class ExportsService {
    static async exportReadings(user, farmZoneId, from, to) {
        ...
        if (from || to) {
            ...
            if (from)
                whereClause.recordedAt.gte = new Date(from);
            if (to)
                whereClause.recordedAt.lte = new Date(to);
        }
        const readings = await prisma_1.default.sensorReading.findMany({
            where: whereClause,
            include: {
                sensor: true,
                farmZone: true
    ...
    static async exportAlerts(user, from, to) {
        const whereClause = {
